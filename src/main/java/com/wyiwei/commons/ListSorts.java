...
public class ListSorts {
	 ...
	 */
	public static <T> void sort(List<T> list, final boolean strIgnorCase, final boolean isAsc, final String... sortName) {
		Collections.sort(list, new Comparator<T>() { 
			public int compare(T a, T b) {
				int ret = 0;
				try {
					for (int i = 0; i < sortName.length; i++) {
						ret = ListSorts.compareObject(sortName[i], strIgnorCase, isAsc, a, b);
						if (0 != ret) {
							break;
						}
					}
	 ...
	 */
	public static <T> void sort(List<T> list,  final boolean strIgnorCase, final String[] sortNameArray, final boolean[] sortDirectionArray) {
			...
			public int compare(T t1, T t2) {
				...
				try {
					...
					for (int i = 0; i < sortNameArray.length; i++) {
						ret = compareObject(sortNameArray[i], strIgnorCase, sortDirectionArray[i], t1, t2);
						if (0 != ret) {
							break;
						}
					}
	 ...
	 * @throws Exception
	 */
	private static <T> int compareObject(final String sortName, boolean strIgnorCase, final boolean isAsc, T t1, T t2) throws Exception {
		int ret;
		...
		} else if (value1 instanceof String && value2 instanceof String && strIgnorCase) {
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<20; i++) {
			SortBean sBean = new SortBean();
			sBean.name = "name"+random.nextInt(5);
			sBean.age = random.nextInt(5);
			sBean.addr = "addr"+random.nextInt(5);
			list.add(sBean);
		}
		final String[] fieldsName = {"name", "age", "addr"};
		final boolean[] dire = {true, false, true};
		sort(list, false, fieldsName, dire);
	...
	static class SortBean {
		...
		public String toString() {
			return MessageFormat.format("{0}, {1}, {2}", name, age, addr);
		}
	}
